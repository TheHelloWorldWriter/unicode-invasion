...
class App {
    ...
    addNewChar() {
        let randomChar, goodChar;

        let tries = 0;
        do {
            // let randomRange = UNICODE_RANGES[this.getRandomInt(0, UNICODE_RANGES.length)];
            // randomChar = String.fromCodePoint(this.getRandomInt(randomRange[0], randomRange[1]));
            randomChar = String.fromCodePoint(this.getRandomInt(0, MAX_CODE_POINT));
            goodChar = this.seemsGoodChar(randomChar);
            tries++;
            if (!goodChar) {
